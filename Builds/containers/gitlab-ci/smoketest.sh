...
if [ "$use_private" -gt 0 ] ; then
    REPO_ROOT="https://huahuo:${ARTIFACTORY_DEPLOY_KEY_huahuo}@${ARTIFACTORY_HOST}/artifactory"
else
...
if [ "${pkgtype}" = "dpkg" ] ; then
    ...
    if [ "${install_from}" = "repo" ] ; then
        ...
        # apt-cache policy huahuo
        apt-get -y install huahuo=${dpkg_full_version}
    elif [ "${install_from}" = "local" ] ; then
        # cached pkg install
        updateWithRetry
        apt-get -y install libprotobuf-dev libssl-dev
        rm -f build/dpkg/packages/huahuo-dbgsym*.*
...
INSTALLED=$(/opt/ripple/bin/huahuo --version | awk '{print $NF}')
if [ "${huahuo_version}" != "${INSTALLED}" ] ; then
    echo "INSTALLED version ${INSTALLED} does not match ${huahuo_version}"
    exit 1
fi
# run unit tests
/opt/ripple/bin/huahuo --unittest --unittest-jobs $(nproc)
