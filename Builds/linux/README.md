...
This document focuses on building huahuo for development purposes under recent
Ubuntu linux distributions. To build huahuo for Redhat, Fedora or Centos
builds, including docker based builds for those distributions, please consult
the [huahuo-package-builder](https://github.com/ripple/huahuo-package-builder)
repository. 
...
At this time, huahuo only supports protobuf version 2. Using version 3 of 
...
Source code documentation is not required for running/debugging huahuo. That
...
### Clone the huahuo repository

From a shell:

```
git clone git@github.com:ripple/huahuo.git
cd huahuo
```

For a stable release, choose the `master` branch or one of the tagged releases
listed on [GitHub](https://github.com/ripple/huahuo/releases). 
...
When the build completes succesfully, you will have a `huahuo` executable in
...
The huahuo cmake build supports an installation target that will install
huahuo as well as a support library that can be used to sign transactions. In
...
huahuo project that you can link to in your own project's CMakeLists.txt, e.g.:
...
First, add the huahuo repo as a submodule to your project repo:

```
git submodule add -b master https://github.com/ripple/huahuo.git vendor/huahuo
```

change the `vendor/huahuo` path as desired for your repo layout. Furthermore,
change the branch name if you want to track a different huahuo branch, such
as `develop`.
   
Second, to bring this submodule into your project, just add the huahuo subdirectory:

```
add_subdirectory (vendor/huahuo)
```
    
##### Option 2: installed huahuo + find_package

First, follow the "Optional Installation" instructions above to
build and install the desired version of huahuo.
...
`huahuo` builds a set of unit tests into the server executable. To run these unit
tests after building, pass the `--unittest` option to the compiled `huahuo`
