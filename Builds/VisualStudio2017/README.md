...
We do not recommend Windows for huahuo production use at this time. Currently,
...
modification, build huahuo under Visual Studio, and run the unit tests you will
...
Any version of Visual Studio 2017 may be used to build huahuo. The **Visual
...
* NOTE: Since huahuo links statically to OpenSSL, it does not matter where the
  OpenSSL .DLL files are placed, or what version they are. huahuo does not use
...
The huahuo application is linked statically to the standard runtimes and
...
process and clone `git@github.com:ripple/huahuo.git`. Otherwise follow this
...
above. Navigate to the directory where you want to clone huahuo (git bash uses
...
listed on [huahuo's GitHub page](https://github.com/ripple/huahuo/releases). 
...
5. Select either the `huahuo.exe` (unity) or `huahuo_classic.exe` (non-unity)
   ...
   huahuo repo, e.g. `datagen.vcxproj`. Please ignore them.
...
```
.\build\x64-Release\Release\huahuo.exe
```
or
```
.\build\x64-Debug\Debug\huahuo.exe
```
...
execute the following commands within your `huahuo` cloned repository:
...
Navigate to the `build\cmake` folder created above and select the `huahuo.sln`
file. You can then choose whether to build the `Debug` or `Release` solution
configuration.

The executable will be in 
```
.\build\cmake\Release\huahuo.exe
```
or
```
.\build\cmake\Debug\huahuo.exe
```
These paths are relative to your cloned git repository.

# Unity/No-Unity Builds

The huahuo build system defaults to using [unity source files](http://onqtam.com/programming/2018-07-07-unity-builds/)
...
`huahuo` builds a set of unit tests into the server executable. To run these
unit tests after building, pass the `--unittest` option to the compiled
`huahuo` executable. The executable will exit with summary info after running
the unit tests.
