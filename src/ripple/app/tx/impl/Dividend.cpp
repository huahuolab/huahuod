...
Dividend::preflight (PreflightContext const& ctx)
{
    ...
    if (!ctx.tx.isFieldPresent (sfDividendCoinsHDU))
    {
        JLOG(ctx.j.warning) << "No dividend coins HDU";
        return temINVALID;
    }
...
TER Dividend::preclaim (PreclaimContext const &ctx)
{
    ...
    if (divType == DividendMaster::DivType_Apply)
    {
        ...
        if (!ctx.tx.isFieldPresent (sfDividendCoinsHDURank))
        {
            JLOG(ctx.j.warning) << "No dividend coins by rank";
            return temINVALID;
        }
        if (!ctx.tx.isFieldPresent (sfDividendCoinsHDUSprd))
        {
...
TER Dividend::startCalc ()
{
    ...
    uint64_t dividendCoinsHDU = tx.getFieldU64 (sfDividendCoinsHDU);
    ...
    dividendObject->setFieldU64 (sfDividendCoinsHDU, dividendCoinsHDU);
...
TER Dividend::applyTx ()
{
    ...
    uint64_t divCoinsHDU = tx.getFieldU64 (sfDividendCoinsHDU);
    ...
    if (sleAccoutModified)
    {
        if (divCoinsHDU > 0)
        {
            sleAccoutModified->setFieldAmount (sfBalanceHDU,
                sleAccoutModified->getFieldAmount (sfBalanceHDU) + divCoinsHDU);
            ctx_.createHDU (divCoinsHDU);
        }
        if (divCoins > 0)
        {
