 ...
 * By using HDU for bridging the most complicated path expected is usually:
 * source -> gateway -> HDU -> gateway -> destination
3. Pareto principle.
 * Good liquidity is often reusable.
...
If you're sending and receiving HDU you don't need a path.
...
2. Build a payment transaction containing the path set.
  ...
  * A payment transaction includes the following fields
    ...
    * destination amount
      ...
      * if currency not HDU, must have a specified issuer.
